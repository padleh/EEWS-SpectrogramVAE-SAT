...
class SpectrogramVAE(keras.Model):
    ...
    def _build_decoder(self):
        """Build the decoder network for spectra prediction"""
        latent_inputs = keras.Input(shape=(self.latent_dim,), name="z_sampling")
        
        # Dense layers
        x = layers.Dense(512, kernel_regularizer=keras.regularizers.l2(1e-5))(latent_inputs)
        x = layers.LeakyReLU(alpha=0.2)(x)
        x = layers.BatchNormalization()(x)
        x = layers.Dropout(0.25)(x)
        
        x = layers.Dense(384, kernel_regularizer=keras.regularizers.l2(1e-5))(x)
        x = layers.LeakyReLU(alpha=0.2)(x)
        x = layers.BatchNormalization()(x)
        x = layers.Dropout(0.25)(x)
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            # Computing KL divergence with numerical stability
            ...
            epsilon = 1e-8
            # Ensure losses are finite - Use TensorFlow ops instead of Python control flow
            ...
            # If nan_or_inf_detected, log a message (outside of graph)
        ...
        # Apply gradients only if they are finite - Use TensorFlow ops for each gradient
        valid_gradients = []
        for grad in grads:
            if grad is not None:
    ...
    def test_step(self, data):
        ...
        # Make sure z_log_var doesn't have extreme values
        z_log_var_clipped = tf.clip_by_value(outputs["z_log_var"], -10.0, 10.0)
        ...
        # Ensure losses are finite - Use TensorFlow ops
...
def train_vae(hdf5_path, images_folder, base_folder, model_folder, history_folder, evaluation_folder,  
    ...
    """
    image_size = (128, 512)  # Resizing spectrograms to this size
    ...
    with h5py.File(hdf5_path, 'r') as f:
        ...
        for i in range(min(3, len(metadata))):
            ...
            # Print all fields from metadata
    ...
    val_end = int(0.9 * total_samples)  # 75% + 15% = 90%
    
    train_indices = indices[:train_end]
    val_indices = indices[train_end:val_end]
    test_indices = indices[val_end:]  # Last 10%
    ...
    test_indices_path = os.path.join(evaluation_folder, f"test_indices_{trim_time}.npy")
    ...
    callbacks = [
        keras.callbacks.ModelCheckpoint(
            filepath=os.path.join(base_folder, f"vae_model_best_trimmed_{trim_time}"),
            save_best_only=True,
            monitor="val_loss",
            save_format="tf"
        ),
        ...
        keras.callbacks.TensorBoard(
            log_dir=os.path.join(base_folder, f"vae_logs_trimmed_{trim_time}"),
            histogram_freq=1
        )
    ]
    ...
    # Save the model
    vae.save(os.path.join(model_folder, f"vae_model_{trim_time}"), save_format="tf")

    # Save history data as CSV
    history_df = pd.DataFrame(history.history)
    history_df.to_csv(os.path.join(history_folder, f"vae_history_{trim_time}.csv"), index=False)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(history_folder, f"vae_training_history_{trim_time}.png"))
    plt.close()
    ...
    with h5py.File(hdf5_path, 'r') as f:
        ...
        for i, idx in enumerate(eval_indices):
            ...
            try:
                ...
                if true_max - true_min > 1e-8:
                    ...
                    true_spectra_norm = true_spectra_log  
                ...
                if idx in plot_indices:
                    plot_idx = np.where(plot_indices == idx)[0][0]  
                    ...
                    ax1.set_title(f"Full Trace - {event_id} | {station}: {magnitude} {mag_type} | Ep: {epicenter} km | Dep: {depth} km")
                    ...
                    ax2.set_title(f"Response Spectra - {event_id} | {station}: {magnitude} {mag_type} | Ep: {epicenter} km | Dep: {depth} km")
                    ...
                    ax3.set_title(f"Trimmed ({trim_time}) Spectrogram - {event_id} | {station}: {magnitude} {mag_type} | Ep: {epicenter} km | Dep: {depth} km")
                    ax3.axis('off')
        ...
        if all_true_norm and all_pred_norm:
            try:
                ...
                excel_path = os.path.join(evaluation_folder, f'vae_evaluation_{trim_time}.xlsx')
        ...
        plt.savefig(os.path.join(evaluation_folder, f"vae_predictions_{trim_time}.png"))
        plt.close()
